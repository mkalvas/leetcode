...
mod test {
    ...
    #[test]
    fn array_of_1_1() {
        assert_eq!(max_subarrays(vec![1]), 1);
    }

    #[test]
    fn array_of_1_0() {
        assert_eq!(max_subarrays(vec![0]), 1);
    }

    #[test]
    fn array_of_2_0() {
        assert_eq!(max_subarrays(vec![0, 0]), 2);
    }

    #[test]
    fn array_of_zero_still_max_one() {
        assert_eq!(max_subarrays(vec![0, 1, 3, 5, 7]), 1);
    }

    #[test]
    fn array_of_zero_still_max_one_palindrome() {
        assert_eq!(max_subarrays(vec![7, 5, 3, 1, 0, 1, 3, 5, 7]), 1);
    }

    #[test]
    fn array_of_zero_palindrome_split() {
        assert_eq!(max_subarrays(vec![0, 7, 5, 3, 1, 0, 1, 3, 5, 7, 0]), 3);
    }

    #[test]
    fn array_of_1_0_3() {
        assert_eq!(max_subarrays(vec![1, 0, 3]), 1);
    }

    #[test]
    fn array_absorbs_tail_when_needed() {
        assert_eq!(max_subarrays(vec![0, 0, 5]), 2);
    }
}
